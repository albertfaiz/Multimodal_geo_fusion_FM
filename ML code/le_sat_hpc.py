...
X_combined = X_combined.clip(lower=X_combined.quantile(0.01), upper=X_combined.quantile(0.99), axis=1)
...
# ============================================================================
# ============================================================================
# PHASE 2: PRODUCTION ML ANALYSIS (SHAP, PRUNING, TABLES)
# ============================================================================
# ============================================================================
print("\n\n" + "="*80)
...
X_raw = X_raw.clip(lower=X_raw.quantile(0.001), upper=X_raw.quantile(0.999), axis=1)
...
X_shap_sample = X.sample(n=min(SHAP_SAMPLE_SIZE, len(X)), random_state=42)
explainer = shap.TreeExplainer(final_model, data=X.sample(n=min(SHAP_BACKGROUND_SIZE, len(X)), random_state=42), feature_perturbation='interventional')
...
print(f"  Running parallel SHAP on pruned feature space ({len(X.columns)} features)...")
...
shap_importance = pd.DataFrame({'Feature': X.columns, 'Mean_|SHAP|': np.abs(shap_values).mean(0), 'RF_Importance': final_model.feature_importances_}).sort_values('Mean_|SHAP|', ascending=False)
...
county_metrics['LE_Bracket'] = pd.cut(county_metrics['actual'], bins=[60, 70, 74, 76.5, 79, 88], labels=['60-70', '70-74', '74-76.5', '76.5-79', '79-88'])
